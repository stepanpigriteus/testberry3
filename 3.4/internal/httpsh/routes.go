...
func RegisterRoutes(mux *http.ServeMux, handlers domain.Handlers) {
	routes := []Route{
		{Method: http.MethodPost, Path: "/upload", Handler: handlers.Upload},
		{Method: http.MethodGet, Path: "/image/", Handler: handlers.Get},
		{Method: http.MethodDelete, Path: "/image/", Handler: handlers.Delete},
	}
